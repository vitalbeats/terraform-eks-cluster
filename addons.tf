resource "null_resource" "calico" {
  ...
  provisioner "local-exec" {
    command = "kubectl apply -f https://raw.githubusercontent.com/aws/amazon-vpc-cni-k8s/release-1.5/config/v1.5/calico.yaml --kubeconfig ${path.root}/output/${var.cluster-name}/kubeconfig-${var.cluster-name}"
  }
